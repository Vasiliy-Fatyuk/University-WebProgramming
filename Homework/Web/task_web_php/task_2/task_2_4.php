	...
	<div class="container bloc-lg">
		<div class="row">
			<div class="col-sm-12">
				...
				<p>
					Для файлу R1, компонентами якого є дійсні числа, скласти програму, що знаходить максимум і мінімум серед кожних десяти компонент файлу R1 і формують з них файл R2.<br>
				</p>
			</div>
		</div>
	</div>
	...
	<div class="container bloc-lg">
		<div class="row">
			<div class="col-sm-12">
				...
				<p class=" text-left">
					&lt;?php<br><br>$R1_as_array = file("R1.txt");<br><br>$result = "";<br>for ($i=0; $i &lt; count($R1_as_array); $i += 10) {<br> if ($i + 10 &gt; count($R1_as_array)) break;<br> $min_element = $R1_as_array[$i];<br> $max_element = $R1_as_array[$i];<br> for ($j = $i; $j &lt; $i + 10; $j++) {<br> if ($min_element &lt; $R1_as_array[$j]) $min_element = $R1_as_array[$j];<br> if ($max_element &gt; $R1_as_array[$j]) $max_element = $R1_as_array[$j];<br> }<br> $result.=$min_element;<br> $result.=PHP_EOL;<br> $result.=$max_element;<br> $result.=PHP_EOL;<br>}<br><br>$file = fopen('R2.txt', 'w+');<br>$file_write = fwrite($file, $result);<br>fclose($file);<br>?&gt;<br>&lt;h2&gt;Файл записано&lt;/h2&gt;<br>
				</p>
			</div>
		</div>
	</div>
