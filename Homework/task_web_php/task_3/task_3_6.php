	...
	<div class="container bloc-lg">
		<div class="row">
			<div class="col-sm-12">
				...
				<p class=" text-left">
                    <br>&lt;?php<br>$arr = array(<br> array(<br> 'text' =&gt; "Test1",<br> 'cells' =&gt; '1,2,3,4,6,7,8,9',<br> 'align' =&gt; 'right',<br> 'valign' =&gt; 'top',<br> 'color' =&gt; 'red',<br> 'bgcolor' =&gt; 'blue'<br> ),<br> array(<br> 'text' =&gt; "Test2",<br> 'cells' =&gt; '13,14,15,18,19,20,23,24,25',<br> 'align' =&gt; 'center',<br> 'valign' =&gt; 'middle',<br> 'color' =&gt; 'blue',<br> 'bgcolor' =&gt; 'red'<br> ),<br>);<br>function getTable(array $arr){<br>// Створюю масив з номерів осередків які ввів користувач в массівв $ arr під<br>// ключем cells і тут же його сортую в порядку зростання<br>for($i = 0;$i &lt; count($arr);$i++)<br>{<br> $delimiter =',';<br> $arr_cells[] = explode($delimiter, $arr[$i]['cells']);<br> sort($arr_cells[$i]);<br>}<br>// КІНЕЦЬ Створення масиву з номерів, які ввів користувач<br>$size =5; // Размер Таблицы = $size*$size.<br>// МОЖНА вибирати будь-який розмір матриці.<br><br>static $k=1;<br>// Під кожен елемент створюю окремий масив<br>$colspan = array();<br>$rowspan = array();<br>$width = array();<br>$height = array();<br>$class = array();<br>$color = array();<br>$bgcolor = array();<br>$text = array();<br>// Заповнюю тадліцу даними за замовчуванням<br>for($i=1; $i &lt;= $size*$size; $i++)<br>{<br> $width[] = '100';<br> $height[] = '100';<br> $colspan[] = '1';<br> $rowspan[] = '1';<br> $text[] = $i;<br>}<br>// Основний код програми<br><br>for($i=0;$i&lt;count($arr_cells); $i++)<br>{<br> // запам'ятовуючи для кожної групи осередків потрібні дані<br> $color[$arr_cells[$i][0]-1] = $arr[$i]['color'];<br> $bgcolor[$arr_cells[$i][0]-1] = $arr[$i]['bgcolor'];<br> $text[$arr_cells[$i][0]-1] = $arr[$i]['text'];<br> $align[$arr_cells[$i][0]-1] = $arr[$i]['align'];<br> $valign[$arr_cells[$i][0]-1] = $arr[$i]['valign'];<br><br> $count =count($arr_cells[$i]);<br> $row=1;<br> for($s= $count-2 ,$j=1;$j&lt;$count,$s &gt;= 0; $j++, $s--)<br> {<br> $class[$arr_cells[$i][$j]-1] = 'hidden';// для непотрібних мені осередків привласнюю клас<br> // Установлюю число осередків, які повинні бути об'єднані по вертикалі.<br> if(($arr_cells[$i][$count -$j] - $arr_cells[$i][$s]) != 1 )<br> {<br> $row++;<br> }<br> }<br> $col =$count/$row;// Установлюю число осередків, які повинні бути об'єднані по горизонталі<br> $colspan[$arr_cells[$i][0]-1] = $col;<br> $rowspan[$arr_cells[$i][0]-1] = $row;<br> // Перевірка того якщо номери осередків йдуть підряд<br> for($l=1;$l&lt;$count;$l++)<br> {<br> if((max($arr_cells[$i])-min($arr_cells[$i])) == $l*$size-1)<br> {<br> $rowspan[$arr_cells[$i][0]-1] = $count/$size;<br> $colspan[$arr_cells[$i][0]-1] = $size;<br> }<br> }<br> // КІНЕЦЬ Перевірки того якщо номери осередків йдуть підряд<br>}<br>// КІНЕЦЬ Основного коду програми<br>?&gt; <br> <br> <br>&nbsp;<br>&lt;div class=" container-fluid"&gt;<br><br> &lt;div class=" col-lg-&lt;?php echo$size?&gt;"&gt;<br> &lt;table class="table table-bordered"&gt;<br> &lt;?php for($i=0;$i&lt;$size;$i++)<br> {?&gt;<br> &lt;tr&gt;<br> &lt;?php for($j=0;$j&lt;$size;$j++,$k++){ ?&gt;<br> &lt;td colspan="&lt;?php echo $colspan[$k-1]?&gt;"<br> rowspan="&lt;?php echo $rowspan[$k-1]?&gt;"<br> class="&lt;?php echo $class[$k-1]?&gt;"<br> style="<br> width: &lt;?php echo $colspan[$k-1]*$width[$k-1].'px';?&gt;;<br> height: &lt;?php echo $rowspan[$k-1]*$height[$k-1].'px';?&gt;;<br> background: &lt;?php echo $bgcolor[$k-1]?&gt;;<br> color: &lt;?php echo $color[$k-1]?&gt;;<br> text-align: &lt;?php echo $align[$k-1]?&gt;;<br> vertical-align: &lt;?php echo $valign[$k-1]?&gt;;<br> "&gt;<br> &lt;?php echo $text[$k-1];?&gt;&lt;/td&gt;<br> &lt;?php }?&gt;<br> &lt;/tr&gt;<br> &lt;?php }?&gt;<br> &lt;/table&gt;<br> &lt;/div&gt;<br><br><br> &lt;?php }?&gt; &lt;! - КІНЕЦЬ тіла функції getTable -&gt;<br><br> &lt;?php getTable($arr); ?&gt; &lt;! - Виклик функції getTable ($ arr) -&gt;<br><br> &lt;!-- &lt;/div&gt;--&gt;<br> &lt;/div&gt;<br>
                </p>
			</div>
		</div>
	</div>
